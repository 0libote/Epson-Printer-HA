document.querySelectorAll('[data-file-input]').forEach(input => {
  const error = document.getElementById('file-inline-error');
  ...
  const validate = () => {
    ...
    label.textContent = file?.name || 'Choose a file';
    if (!file) {
    ...
    // ponytail: show validation immediately
...
// ponytail: live polling for HomeLab validation without page reload (3s)
(() => {
  const pollInterval = Number.parseInt(document.body.dataset.pollInterval || '3000', 10);
  if (!document.body.dataset.printerIp) return;
  ...
  let lastQueue = null, lastHistory = null;
  ...
  async function poll() {
    try {
      ...
      if (summaryPrinterText) summaryPrinterText.textContent = 'Printer ' + pState.replace('_', ' ');
      if (statusPrinterText) statusPrinterText.textContent = title(pState.replace('_', ' '));
      ...
      if (queueList) {
        ...
        if (qJson !== lastQueue) {
          ...
          queue.forEach(job => {
            ...
            // ponytail: simple text rendering, no innerHTML injection risk beyond job.id which is validated server-side
          ...
          if (queuePanel) queuePanel.hidden = qCount === 0;
          if (activityGrid) {
            const scansVisible = scansPanel && !scansPanel.hidden;
            activityGrid.hidden = qCount === 0 && !scansVisible;
          }
        }
      }

      // scans polling (validation that scan succeeded)
      if (scansList) {
        const scans = Array.isArray(data.scans) ? data.scans : [];
        const sJson = JSON.stringify(scans);
        // ponytail: simple scans cache to avoid DOM churn
        if (sJson !== (scansList.dataset.last || '')) {
          scansList.dataset.last = sJson;
          scansList.innerHTML = '';
          scans.forEach(name => {
          ...
          if (scansPanel) scansPanel.hidden = scans.length === 0;
          if (activityGrid) {
            const qVisible = queuePanel && !queuePanel.hidden;
            activityGrid.hidden = scans.length === 0 && !qVisible;
          }
        }
      }

      // history: use full 100 via /api/history when tbody exists, fallback to recent_prints
      try {
        if (historyTbody) {
          // ponytail: fetch full history every poll (homelab LAN, tiny payload) for true validation
          const hRes = await fetch('/api/history?limit=100', { credentials: 'same-origin', headers: { Accept: 'application/json' } });
          if (hRes.ok) {
            const hData = await hRes.json();
            const prints = Array.isArray(hData.history) ? hData.history : [];
            const hJson = JSON.stringify(prints.map(p => p.history_key || p.job_id + ':' + p.created_at + ':' + p.state));
          ...
          const prints = Array.isArray(data.recent_prints) ? data.recent_prints : [];
  ...
  // initial poll shortly after load, then interval
  setTimeout(poll, 800);
  const timer = setInterval(poll, pollInterval);
  // pause polling when tab hidden to save homelab resources
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) clearInterval(timer);
    else { poll(); setInterval(poll, pollInterval); }
  });
})();
